...
It stores tickets, replies, final results, and attached docs in one SQLite file.
...
- A durable log of requests, replies, final results, and embedded docs.
...
| `atix done` | Finish claimed work using the receipt from `inbox`. |
| `atix show` | Read a ticket or ask group, optionally waiting for it to close. |

The normal loop is:

```text
send -> inbox -> reply* -> done -> show
```

`send` and `ask` create work. `inbox` receives work and returns a receipt.
`done` requires that receipt so the worker that claimed the ticket is the worker
that closes it.
...
atix show "$ID" --with-replies --with-docs
```
...
Docs are embedded into SQLite at command time. `--doc-file name=path` stores the
file content when `send`, `ask`, `reply`, or `done` runs. It does not store a
durable path reference, so later edits to the file do not change the ticket.
...
Use `show --with-docs` to include docs in JSON output. For individual tickets,
human output lists the attached doc names and sizes.
...
atix show "$GROUP" --with-docs --with-replies --json
```

Each channel receives its own child ticket. `show` accepts either the ask group
ID or an individual ticket ID. Use `--json` when an agent needs the full group,
child ticket, doc, and reply structure.
...
# Wait for one ticket or ask group to close, then return it.
atix show "$ID" --wait 30m --with-replies --with-docs --json
```
...
They are not the normal README path. Prefer `send`, `ask`, `inbox`, `reply`,
`done`, and `show` unless you need a specific primitive behavior.
